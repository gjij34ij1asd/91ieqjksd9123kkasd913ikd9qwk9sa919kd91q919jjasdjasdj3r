...
public class IslandCommand implements CommandExecutor {
    ...
    private void handleCoop(Player player, String targetName) {
        Island island = plugin.getIslandManager().getPlayerIsland(player.getUniqueId());
        if (island == null) {
        ...
        if (target.getUniqueId().equals(player.getUniqueId())) {
            player.sendMessage("§8» §cNie możesz dodać samego siebie!");
            return;
        }

        if (island.getCoopPlayers().contains(target.getUniqueId())) {
            island.getCoopPlayers().remove(target.getUniqueId());
            player.sendMessage("§8» §cUsunięto tymczasowe uprawnienia dla gracza " + target.getName());
            target.sendMessage("§8» §cTwoje tymczasowe uprawnienia na wyspie gracza " + player.getName() + " zostały usunięte!");
        } else {
            island.getCoopPlayers().add(target.getUniqueId());
            player.sendMessage("§8» §aDodano tymczasowe uprawnienia dla gracza " + target.getName());
            target.sendMessage("§8» §aOtrzymałeś tymczasowe uprawnienia na wyspie gracza " + player.getName());
        }
    ...
    private void acceptInvite(Player player) {
        ...
        // Powiadom wszystkich członków wyspy
        for (UUID memberId : island.getMembers()) {
            Player member = Bukkit.getPlayer(memberId);
            if (member != null && !member.equals(owner) && !member.equals(player)) {
                member.sendMessage("§8» §a" + player.getName() + " dołączył do wyspy!");
            }
        }
