    ...
    private void loadEconomy() {
        ...
        try (Reader reader = new FileReader(economyFile)) {
            ...
            if (economyData != null) {
                ...
                Map<UUID, Double> bankMoney = new HashMap<>();

                if (economyData.containsKey("playerMoney")) {
                    economyData.get("playerMoney").forEach((uuidStr, amount) ->
                            playerMoney.put(UUID.fromString(uuidStr), amount));
                }

                if (economyData.containsKey("bankMoney")) {
                    economyData.get("bankMoney").forEach((uuidStr, amount) ->
                            bankMoney.put(UUID.fromString(uuidStr), amount));
                }

                plugin.getEconomyManager().loadFromMap(playerMoney, bankMoney);
            }
        } catch (Exception e) {
